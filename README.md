...
```text
LuminaLink-Setup-0.1.3-x64.exe       安装版
LuminaLink-Portable-0.1.3-x64.exe    免安装版
```
...
- 如果不想配置 Provider，可以用 CLI 导出“智能体翻译任务包”，让 Codex 这类本机智能体翻译后再导入缓存。
...
不配置 Provider，交给 Codex 或其他智能体翻译：

```bash
pnpm luminalink translate export-task "%TEMP%/luminalink-translation-task.json" --limit 10
```

让智能体把 JSON 里的 `items[].translatedText` 填成简体中文后导入：

```bash
pnpm luminalink translate import-result "%TEMP%/luminalink-translation-task.json"
```
